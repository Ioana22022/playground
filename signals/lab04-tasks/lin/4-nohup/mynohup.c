...
void do_redirect(int fd, char *filename){
	//redirect fd into filedes
	//int mode = 0666;
	int filedes;
	filedes = open(filename, O_TRUNC | O_CREAT | O_RDWR, 0666);
	dup2(filedes, fd);
	close(filedes);
...
static void exec_func(int argc, char **argv)
{
	 ...
	 */
	if(isatty(STDOUT_FILENO) == 1) do_redirect(STDOUT_FILENO, NOHUP_OUT_FILE);
