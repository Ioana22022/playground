...
static void wait_aio(io_context_t ctx, int nops)
{
	...
	printf("%d operations read", (int)efd_ops);
#endif
	free(events);
}
...
static void do_io_async(void)
{
	...
	iocb = (struct iocb*) malloc (sizeof(struct iocb) * n_files);
	piocb = (struct iocb**) malloc (sizeof(struct iocb *) * n_files);

	memset(iocb, 0, sizeof(struct iocb) * n_files);
