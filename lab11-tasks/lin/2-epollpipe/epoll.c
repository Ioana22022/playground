...
static int server(void)
{
	...
	for (i = 0; i < CLIENT_COUNT; i++) {
		...
		DIE(rc < 0, "close pipes write end");
	...
	// add event_fd descriptor
#ifdef USE_EVENTFD

	ev.data.fd = event_fd;
	ev.events = EPOLLIN;
	printf("ADDING EVENT_FD WITH EPOLL_CTL\n");
	rc = epoll_ctl(efd, EPOLL_CTL_ADD, event_fd, &ev);
	printf("ADDED event_fd:%d", event_fd);
	DIE(rc < 0, "epoll_ctl");
	...
	while (recv_msgs < CLIENT_COUNT) 
	{
		...
		if(ret_ev.data.fd == event_fd)
		{
			...
			recv_count = read(event_fd, &event, sizeof(uint64_t));
			printf("server got Magic EVENT: 0x%lx\n", event);
		...
		if(ret_ev.events & EPOLLIN)
		{
			...
			rc = epoll_ctl(efd, EPOLL_CTL_DEL, ret_ev.data.fd, &ret_ev);
			DIE(rc < 0, "epoll_ctl");
			recv_msgs++;
		}
...
static int client(unsigned int index)
{
	...
	rc = write(event_fd, &event, sizeof(uint64_t));
	DIE(rc < 0, "write eventfd");
