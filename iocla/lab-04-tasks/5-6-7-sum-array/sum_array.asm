...
section .data
    ...
    dword_array_1 dd 1392, 12544, 7992, 6992, 7202, 27187, 28789, 17897, 12988, 17992
...
add_word_array_element:
    ...
    add eax, edx
    loop add_word_array_element ; Decremenent ecx, if not 0, jump back to label
...
add_dword_array_element:
    ...
    add eax, edx
    loop add_dword_array_element ; Decremenent ecx, if not 0, jump back to label
        
    PRINT_STRING print_format
    PRINT_UDEC 4, eax
    NEWLINE
    
   ; Compute sum for elements squared in dword_array_1
    mov ecx, ARRAY_SIZE     ; Use ecx as loop counter.
    xor ebx, ebx            ; Use ebx to store the sum.
    xor edx, edx            ; Store current value in dl; zero entire edx.
    
add_dword_array_square:
    mov edx, dword [dword_array_1 + 4*ecx - 4]
    ;add eax, edx
    mov eax, edx
    mul edx
    add ebx, eax
    loop add_dword_array_square ; Decremenent ecx, if not 0, jump back to label
