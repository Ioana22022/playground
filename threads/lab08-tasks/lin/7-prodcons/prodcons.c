...
void *producer_fn(void *arg)
{
	...
	for (i = 0; i < NR_ITERATIONS; i++) {
		 ...
		 */
		if(is_buffer_full(&common_area)) pthread_cond_wait(&buffer_not_full, &mutex);

		/* TODO - insert item into common area */
		insert_item(&common_area, i);
		...
		if(common_area.count == 1) pthread_cond_signal(&buffer_not_empty);
		/* TODO - unlock mutex */
...
void *consumer_fn(void *arg)
{
	...
	for (i = 0; i < NR_ITERATIONS; i++) {
		...
		if(is_buffer_empty(&common_area)) pthread_cond_wait(&buffer_not_empty, &mutex);
		...
		if(common_area.count == (BUFFER_SIZE - 1)) pthread_cond_signal(&buffer_not_full);
		/* TODO - unlock mutex */
		pthread_mutex_unlock(&mutex);
